...
export class CreateUserComponent implements OnInit {
  ...
  agregarUsuario() {
    ...
    if(this.id !==null){
      ...
      this._usuarioService.editarProducto(this.id, USUARIO).subscribe(data => {
        this.toastr.info('El visitante fue actualizado con exito!', 'VISITANTE ACTUALIZADO!');
      ...
      this._usuarioService.guardarUsuario(USUARIO).subscribe(data => {
          this.toastr.success('El visitante fue registrado con exito!', 'VISITANTE REGISTRADO!');
          this.router.navigate(['/']);
        }, error => {
          console.log(error);
          this.userForm.reset();
  ...
  esEditar() {
    ...
    if (this.id !== null) {
      this.titulo = 'EDITAR DATOS DE VISITANTE.';
